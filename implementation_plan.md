...
**Status: 2 of 6 done** (Cancel UI, Mock mode) — the other four (activity feed, analytics panel, USD-value context, `.cook` names) are pending a scope decision with the user rather than being built by default, per this phase's own "build whichever fit" framing.
...
- **Mock mode** (`?mock=1`) turned out more valuable than expected given Phase 5 is still paused: it's a synthetic Wallet Standard wallet (`lib/mock.ts`) backed by a freshly generated, unfunded keypair, appended to the real wallet list by `listSolanaWallets()` — meaning it plugs into all of the existing connect/reconnect/sign machinery with zero changes to `WalletContext.tsx`. Only the *wallet* is faked; reads still hit the real Cookie Chain RPC (a genuinely useful check, not a shortcut), and a real write would correctly fail with "insufficient funds" once it reached the network rather than something needing to be faked too. Verified end-to-end with the headless-browser method: connects instantly, shows a real (correctly zero) balance from the live network, silently reconnects across a full page reload via the same localStorage mechanism real wallets use, and `MyVaults` correctly queries the live network and renders "No vaults yet" — which also incidentally confirms `getProgramAccounts`-based queries degrade gracefully (empty array, not an error) against a program that isn't deployed yet, rather than crashing. A persistent banner makes mock mode impossible to mistake for the real thing.
