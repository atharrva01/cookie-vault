...
**Status: built, verified as far as possible without a live deployment or a wallet.** All three planned files exist, plus two additions not in the original file list:
- **`lib/router.ts`** — a tiny hash router (`useRoute`/`navigate`), added because Create Vault is the first real "page" and something has to route to it. Not react-router-dom, deliberately — same reasoning PayJar's real, working submission used: this deploys as a static site, a hash route needs no server rewrite rules for a direct link or refresh to keep working, and 4 screens doesn't justify the dependency.
- **`lib/transact.ts`** (`sendWithStatus`) — the piece that actually drives the four `TxStatus` phases. Getting building/awaiting-signature/submitted/confirmed as real distinct moments meant *not* calling Anchor's `.rpc()` (which collapses sign+send+confirm into one opaque await) — `program.ts`'s four action functions were refactored to return an unsigned `Transaction` each (renamed `xxxTx`) instead of calling `.rpc()` themselves, so `sendWithStatus` can sign, send, and confirm as separate awaited steps with a phase callback between each. This only strictly needed to happen for `initializeVaultTx` this phase, but `claimTx`/`approveMilestoneTx`/`cancelVaultTx` got the same treatment now rather than leaving three functions to redo identically in Phase 8.
- `txErrorMessage`/`TxPhase` ended up in `lib/txStatus.ts` rather than living in `components/TxStatus.tsx` (same fast-refresh-driven split as `useWallet.ts` in Phase 6).

**DAS endpoint shape couldn't be verified against official docs** — `api.cookiescan.io` 403s the fetch tooling available here (blocks non-browser requests), and `docs.cookiechain.wtf` has no dedicated DAS API page. `das.ts` uses the endpoint shape confirmed from a real, working Cookie Chain submission's actual source (`GET /api/tokens`, full registry fetched once and searched client-side; on-chain fallback via `getParsedAccountInfo` for a valid mint the registry doesn't know about, since `decimals` is required regardless of registry coverage). Worth a quick sanity check once this is testable in an actual browser, since it was inferred rather than doc-verified.

**Verified with the same headless-Chromium method as Phase 6** (Playwright installed just for the check, removed after): zero console/page errors on `/#/create`, time-lock and milestone modes both render and update correctly (milestone total auto-sums live as amounts are typed — 250 + 750 correctly showed "Total to lock: 1,000 COOK"), and the submit button is correctly `disabled` with no wallet connected. What's *not* verified, same caveat as Phase 6: an actual submitted transaction, a real DAS registry response, or the on-chain error round-trip — all need a live deployment and a real wallet, both still pending Phase 5's funding.

One real bug caught and fixed during this phase, not just written and left: `das.ts`'s mint-resolution cache originally stored a null-converted promise separately from the raw cached one — a second caller hitting the cache during the brief window before failure-cleanup ran could get a thrown error instead of `null`, breaking the "returns null for an invalid mint" contract non-deterministically. Fixed by caching only the raw promise and having every caller (cache hit or miss) derive its own `.catch(() => null)` from it.
...
**Status: built, and this phase caught a real bug the headless-browser check was specifically worth running for.** All three planned files exist (`MyVaults.tsx`, `VaultDetail.tsx`, `components/ui.tsx`), plus `fetchVaultsFor` added to `program.ts` (two `memcmp`-filtered `.all()` calls, one on `depositor` at byte offset 8, one on `recipient` at offset 40 — both safe to hand-derive since they're the first two fields on `Vault` and both fixed-size `Pubkey`s; nothing past `recipient` should ever be memcmp'd this way, since every later field sits behind a variable-length `Vec`) and `buildReadOnlyProvider` added to `idl.ts` (vault state is public on-chain data per design_doc.md §2 — viewing it shouldn't require a connected wallet, but `AnchorProvider` still needs *some* wallet-shaped object, so this one's signing methods throw instead of a hacky dummy signer).

**The bug**: `VaultDetail.tsx` derived `pda` as `new PublicKey(idParam)` directly in the render body, then used `pda` as a `useEffect` dependency in the vault-fetching hook. A `PublicKey` object gets a new identity every render even when the underlying address string hasn't changed, so React saw a "changed" dependency on every render and re-ran the fetch effect every time — each fetch's cleanup cancelled the *previous* one before it could ever resolve, so the page hung on "Loading…" forever for any vault. Direct Node testing of the same `fetchVault` call (bypassing React entirely) resolved correctly in ~700ms, which is what made clear this was a React rendering bug, not a network or RPC issue. Fixed with `useMemo(() => ..., [idParam])`, keying the memo on the stable string instead. This is exactly the kind of bug that a pure code read or a typecheck cannot catch — it only showed up by actually running the page and watching it hang.

**Verified with the same headless-Chromium method as Phases 6-7**, with one page freshly reconfirmed after the fix: My Vaults (no wallet) renders correctly, Vault Detail's not-found state now resolves in a couple seconds via a *real* round trip against the live Cookie Chain RPC for a randomly-generated address (no wallet needed — this exercises actual network I/O, not a mock), nav links work. Zero console/page errors. What's still not verified, same caveat as every frontend phase so far: a vault that actually exists, claim/approve against real data, or the role-based button visibility (depositor sees Approve only, recipient sees Claim only) — all need Phase 5's live deployment plus real vault data to create first.
...
**Status: all 6 done.** Asked the user to scope this phase rather than assuming; they chose all four remaining items, built in one pass.

- **Cancel UI**: a button on Vault Detail's summary card, visible to the depositor only when `!cancelled && releasedAmount == 0` (mirrors the on-chain guard exactly, so the button's enabled state can never promise something the program will reject), with a `window.confirm` since it's a real, if low-risk, action. Reuses `cancelVaultTx`, already written in Phase 7 for exactly this.
- **Mock mode** (`?mock=1`) turned out more valuable than expected given Phase 5 is still paused: it's a synthetic Wallet Standard wallet (`lib/mock.ts`) backed by a freshly generated, unfunded keypair, appended to the real wallet list by `listSolanaWallets()` — meaning it plugs into all of the existing connect/reconnect/sign machinery with zero changes to `WalletContext.tsx`. Only the *wallet* is faked; reads still hit the real Cookie Chain RPC (a genuinely useful check, not a shortcut), and a real write would correctly fail with "insufficient funds" once it reached the network rather than something needing to be faked too. Verified end-to-end: connects instantly, shows a real (correctly zero) balance from the live network, silently reconnects across a full page reload via the same localStorage mechanism real wallets use, and `MyVaults` correctly queries the live network and renders "No vaults yet" — which also confirmed `getProgramAccounts`-based queries degrade gracefully (empty array, not an error) against a program that isn't deployed yet. A persistent banner makes mock mode impossible to mistake for the real thing.
- **USD-value context**: `das.ts`'s registry loader already fetched every token's metadata in one call, so it now also captures each entry's `price.usd` field, plus a dedicated `fetchCookUsdPrice()` (`/api/price/cook`) for native COOK, which isn't itself a registry entry. Vault Detail's Total/Released fields show `(≈ $X)` whenever a price is known — best-effort, silently omitted (not faked) when Cookiescan has no price for that mint.
- **Activity feed**: `lib/activity.ts` pulls `getSignaturesForAddress` on the vault PDA, then decodes each transaction's instruction via a `BorshInstructionCoder` built straight from the program's own IDL — Cookie Vault isn't a program the RPC's transaction parser recognizes, so instructions come back only partially decoded (raw base58 data) without this step. One real API-surface gotcha here: `program.coder.instruction` is typed as the generic `InstructionCoder` interface, which only declares `encode` — `decode` exists on the concrete `BorshInstructionCoder` class but isn't visible through that narrower type, so the coder is constructed directly from the IDL instead of going through `program.coder`.
- **Analytics panel**: `lib/analytics.ts` calls `program.account.vault.all()` with no filter — every vault the program has ever created, not just the connected wallet's — and aggregates vault count, an active/released/cancelled breakdown (rendered as a small dependency-free inline SVG bar chart, no charting library), and total value locked *grouped by mint* rather than summed into one meaningless cross-token number, with an approximate combined USD figure only for the mints a price is actually known for.
- **`.cook` name resolution**: this was the one flagged as having no verified CookOven PDA-derivation seeds — resolved properly rather than guessed. Re-cloned PayJar (the scratchpad had been cleared between sessions) specifically to read its actual `names.ts`, and adapted the verified program ID, account discriminators, and manual bounds-checked byte-layout parsing (it's a raw account from a program we don't own — no IDL, so no shortcut around hand-parsing borsh bytes) into `lib/cookNames.ts`. The Create Vault recipient field now accepts a `bob.cook`-style name or a raw address, resolves it the same debounced way the asset field already does, and shows the resolved address inline.

All of the above were verified with the same headless-Chromium method as every frontend phase: Analytics renders correctly against the live (undeployed) program (0 vaults, not an error), `.cook` name resolution does a real on-chain PDA lookup against CookOven's actual deployed program and correctly reports an unregistered name, Vault Detail's new Activity section renders cleanly alongside the not-found state. Zero console/page errors throughout. Same standing caveat as every frontend phase: claiming/approving/cancelling against a vault that actually exists, and a `.cook` name that's actually registered, both need Phase 5's live deployment (plus, for the latter, someone having registered one) to verify end to end.
