...
fn test_initialize() {
    ...
    let counter = Pubkey::find_program_address(
        &[cookie_vault::constants::COUNTER_SEED],
        &program_id,
    )
    .0;
    ...
    let instruction = Instruction::new_with_bytes(
        program_id,
        &cookie_vault::instruction::Initialize {}.data(),
        cookie_vault::accounts::Initialize {
            payer: payer.pubkey(),
            counter,
    ...
    let counter_state = cookie_vault::state::Counter::try_deserialize(&mut data).unwrap();
    assert_eq!(counter_state.count, 0);
    assert_eq!(counter_state.authority, payer.pubkey());

    let instruction = Instruction::new_with_bytes(
        program_id,
        &cookie_vault::instruction::Increment {}.data(),
        cookie_vault::accounts::Increment {
            counter,
